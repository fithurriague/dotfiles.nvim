    ...
    dependencies = {
      { "AstroNvim/astroui", opts = { icons = { Trouble = "󱍼" } } },
      {
        ...
        opts = function(_, opts)
          ...
          maps.n[prefix .. "s"] = { "<cmd>exe 'Trouble symbols toggle win.position=left focus=true win.size=' . (winwidth(0) * 1/2)<cr>", desc = "Document symbols (Trouble)" }
          maps.n[prefix .. "L"] = { "<cmd>exe 'Trouble lsp toggle win.position=left focus=true win.size=' . (winwidth(0) * 1/2)<cr>", desc = "Lsp inspect all (Trouble)"}
